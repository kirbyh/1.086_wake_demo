...
fi = FlorisInterface(Path(__file__).parent / "inputs/gch.yaml") # GCH model matched to the default "legacy_gauss" of V2
# fi = FlorisInterface("inputs/cc.yaml") # New CumulativeCurl model

# Define a two turbine farm
D = 126.
Sx, Sy = 7 * D, 4 * D
xG, yG = np.meshgrid(np.arange(0, 6) * Sx, np.arange(0, 6) * Sy, indexing='ij')
